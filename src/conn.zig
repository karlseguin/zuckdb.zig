...
test "readonly" {
	{
		// first create a connection that can write since a readonly connection
		// cannot create the DB
		const db = switch(DB.init(t.allocator, "/tmp/duckdb.zig.test", .{})) {
			.ok => |db| db,
			.err => |err| {
				std.debug.print("Failed to open test db '/tmp/duckdb.zig.test': {s}\n", .{err.desc});
				err.deinit();
				unreachable;
			}
		};
		...
		try conn.execZ("drop table if exists readonly_test");
		try conn.execZ("create table readonly_test (id int)");
		try conn.execZ("insert into readonly_test (id) values (91)");
	}


	{
		const db = switch(DB.init(t.allocator, "/tmp/duckdb.zig.test", .{.access_mode = .read_only})) {
			.ok => |db| db,
			.err => |err| {
				std.debug.print("Failed to open test db '/tmp/duckdb.zig.test': {s}\n", .{err.desc});
				err.deinit();
		...
		const err1 = conn.queryZ("drop table readonly_test", .{}).err;
		defer err1.deinit();
		try t.expectEqualStrings("Invalid Error: Cannot execute statement of type \"DROP\" on database \"duckdb\" which is attached in read-only mode!", err1.desc);
		try t.expectEqual(true, err1.err == error.InvalidSQL);

		const err2 = conn.queryZ("insert into readonly_test (id) values (33)", .{}).err;
		defer err2.deinit();
		try t.expectEqualStrings("Invalid Error: Cannot execute statement of type \"INSERT\" on database \"duckdb\" which is attached in read-only mode!", err2.desc);
		try t.expectEqual(true, err2.err == error.InvalidSQL);

		const row = conn.rowZ("select id from readonly_test", .{}).ok.?;
		defer row.deinit();
		try t.expectEqual(@as(i32, 91), row.get(i32, 0).?);
	}
}
